...
Connection::Connection(const char *host, int port)
    : m_ResponseBeginCB(0), m_ResponseDataCB(0), m_ResponseCompleteCB(0),
      m_UserData(0), m_State(IDLE), m_Host(host), m_Port(port), m_Sock(-1) {}
...
Response::Response(const char *method, Connection &conn)
    : m_State(STATUSLINE), m_Connection(conn), m_Method(method), m_Version(0),
      m_Status(0), m_BytesRead(0), m_Chunked(false), m_ChunkLeft(0),
      m_Length(-1), m_WillClose(false) {}
